...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	for k, v := range f.Fields {
		str = append(str, fmt.Sprintf("%s=%+v", colorScheme.DebugLevelColor(k), v))
	}
	fieldsStr := " " + strings.Join(str, " ")

	message := fmt.Sprintf("[%s:%d] [%s] %s%s", file, line, getFuncName(entry.Caller.Function), entry.Message, fieldsStr)
	prefix = colorScheme.PrefixColor(" " + f.Prefix + ":")
